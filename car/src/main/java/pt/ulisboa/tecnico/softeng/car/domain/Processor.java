		...
		for(Renting renting : this.rentingToProcess) {
			if(!renting.isCancelled()) {
				if(renting.getPaymentReference() == null) {
					try {
						renting.setPaymentReference(BankInterface.processPayment(renting.getBuyerIban(), renting.getAmount()));
					} catch (BankException | RemoteAccessException ex) {
						failedToProcess.add(renting);
						continue;
					}
				}
				InvoiceData invoiceData = new InvoiceData(renting.getProviderNif(), renting.getBuyerNif(), renting.getType(), renting.getAmount(), renting.getBegin());
				try {
				...
				try {
					if (renting.getCancelledPaymentReference() == null) {
						renting.setCancelledPaymentReference(BankInterface.cancelPayment(renting.getPaymentReference()));
					}
